...
class BusinessNews : Fragment() {

    var adapter: NewsAdapter? = null
    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    )= inflater.inflate(R.layout.fragment_business_news, container, false)

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        GlobalScope.launch(Dispatchers.Main) {
            val response = withContext(Dispatchers.IO) {
                ClientAPI.api.getBusinessHeadline()
            }
            if (response.isSuccessful) {
                val news: News? = response.body()
                adapter = NewsAdapter(news)
                rvBusinessNews.layoutManager = LinearLayoutManager(requireContext())
                rvBusinessNews.adapter = adapter
            }

            adapter?.onItemClick = {
                Intent(requireContext(), ShortNews::class.java).also { it1->
                    it1.putExtra("ImageURl",it.img_url)
                    it1.putExtra("Category","Business")
