...
class EntertainmentNews : Fragment() {
    ...
    ) = inflater.inflate(R.layout.fragment_entertainment_news, container, false)

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        GlobalScope.launch(Dispatchers.Main) {
            val response = withContext(Dispatchers.IO) {
                ClientAPI.api.getEntertainmentHeadline()
            }
            if (response.isSuccessful) {
                val news: News? = response.body()
                adapter = NewsAdapter(news)
                rvEntertainmentNews.layoutManager = LinearLayoutManager(requireContext())
                rvEntertainmentNews.adapter = adapter
            }

            adapter?.onItemClick = {
                Intent(requireContext(), ShortNews::class.java).also { it1->
                    it1.putExtra("ImageURl",it.img_url)
                    it1.putExtra("Category","Entertainment")
